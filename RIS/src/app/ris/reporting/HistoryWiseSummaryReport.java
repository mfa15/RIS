...
public class HistoryWiseSummaryReport {
...
//	}
	...
	private void build(File f) {
        ...
        try {
            ...
            Logger.getLogger(HistoryWiseSummaryReport.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//                            .highlightDetailEvenRows()
                            ...
                            .columns(//add columns
//                                    /*rowNumberColumn,*/ 
                                    locationColumn,snoColumn,regnoColumn,itemColumn,  quantityColumn,quantityColumn2,quantityColumn3,quantityColumn4,quantityColumn5/*, unitPriceColumn, priceColumn, pricePercColumn*/)
                            .columnGrid(
                                    /*rowNumberColumn,*/locationColumn,snoColumn,regnoColumn,itemColumn, quantityColumn,quantityColumn2,quantityColumn3,quantityColumn4,quantityColumn5, /*unitPriceColumn,*/ grid.verticalColumnGridList(/*priceColumn, pricePercColumn*/))
//                            .groupBy(itemGroup)
...
//                                                cmp.text("abc").setHorizontalTextAlignment(HorizontalTextAlignment.LEFT),
                                    ...
                                    .newRow()
                                    .add(cmp.text("HISTORY WISE SCANS REPORT").setStyle(boldCenteredStyle))
                                    .newRow()
                                    .newRow()
                                    .add(cmp.filler().setStyle(stl.style().setTopBorder(stl.pen2Point())).setFixedHeight(10))
...
//            String gender = bo.getGender().equalsIgnoreCase("M")?"Male":"Female";
		DRDataSource dataSource = new DRDataSource("location","sno","regno","item", "quantity","quantity2","quantity3","quantity4","quantity5");
...
//here
                ...
                dataSource.add("******************","********************","********************","********************");
                dataSource.add("Total Scans","","",String.valueOf(totalScans));
                dataSource.add("******************","********************","********************","********************");
                String type="",print="";
                for(String key:this.patientDetailMap.keySet())
                {
                    ...
                    if(!print.equalsIgnoreCase(""))
                    {
                        ...
                        dataSource.add("******************");
                        dataSource.add("Location","S.NO","REG.NO","MRN","Name","Gender","Age","CT Type","Findings");
                    }
                    String cont = "";
            ...
            dataSource.add(locationName, value.getId(), value.getRegno(), value.getPatientId(), value.getName(),
                   value.getGender() , Constants.calculateAge(value.getDob()), cont, value.getFindings());
...
//	}
    ...
    public String generateOrder(String location,String from,String to,LinkedHashMap<String,PatientSummaryReportBo> patientSummaryMap,LinkedHashMap<String,PatientDetailReportBo> patientDetailMap,String cpt) {
        ...
        this.cpt = cpt;
        File f = new File(System.getProperty("java.io.tmpdir")+"\\temp\\HistorySummary"+System.currentTimeMillis() % 1000+".pdf");
